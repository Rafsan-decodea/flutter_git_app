...
class AuthRepo extends GetxService{
  ...
  Future<Response> checkPhoneNumber({String? phoneNumber}) async {
    return apiClient.postData(AppConstants.agentPhoneCheckUri, {"phone": phoneNumber});
  }

  
  Future<Response> verifyPhoneNumber({String? phoneNumber,String? otp}) async {
    return apiClient.postData(AppConstants.agentPhoneVerifyUri, {"phone": phoneNumber, "otp": otp});
  }

   Future<Response> login({String? dialCode,String? phone, String? password}) async {
     return await apiClient.postData(AppConstants.agentLoginUri, {
   ...
   Future<Response>  checkOtpApi() async {
     return await apiClient.postData(AppConstants.agentCheckOtp,{});
   }

   Future<Response>  verifyOtpApi({required String otp}) async {
     return await apiClient.postData(AppConstants.agentVerifyOtp, {'otp': otp });
   }


   Future<Response> logout() async {
     return await apiClient.postData(AppConstants.agentLogoutUri,{});
   }

   Future<Response> updateProfile(Map<String,String> profileInfo,List<MultipartBody> multipartBody) async {
     return await apiClient.postMultipartData(AppConstants.agentUpdateProfile, profileInfo, multipartBody);
   }

   Future<Response>  pinVerifyApi({required String pin}) async {
     return await apiClient.postData(AppConstants.agentPinVerify,{'pin': pin});
   }
   ...
   Future<Response> forgetPassOtp({String? phoneNumber}) async {
     return apiClient.postData(AppConstants.agentForgetPassOtpUri, {"phone": phoneNumber});
   }
   Future<Response> forgetPassVerification({String? phoneNumber, String? otp}) async {
     return apiClient.postData(AppConstants.agentForgetPassVerification, {"phone": phoneNumber, "otp": otp});
   }

   Future<Response> forgetPassReset({String? phoneNumber, String? otp, String? password, String? confirmPass}) async {
     return apiClient.putData(AppConstants.agentForgetPassReset, {"phone": phoneNumber, "otp": otp, "password": password, "confirm_password": confirmPass});
   }
   ...
   Future<Response> deleteUser() async {
     return await apiClient.deleteData(AppConstants.agentRemove);
   }
