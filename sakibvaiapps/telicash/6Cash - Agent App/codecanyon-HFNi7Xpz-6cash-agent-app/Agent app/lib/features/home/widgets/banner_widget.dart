  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<SplashController>(
        builder: (splashController) {
          return splashController.configModel!.systemFeature!.bannerStatus! ? GetBuilder<BannerController>(builder: (bannerController){
            return bannerController.bannerList == null  ? const Center(child: _BannerShimmerWidget()) :
            (bannerController.bannerList?.isNotEmpty ?? false) ?  Container(
                  ...
                  SizedBox(
                    ...
                    child: CarouselSlider.builder(itemCount: bannerController.bannerList!.length,
                      itemBuilder: (context, index, realIndex) {
                        final String image = (bannerController.bannerList?.isNotEmpty ?? false) ? (bannerController.bannerList?[index].image) ?? '' : '';
                        return InkWell(
                          onTap: (){
                            if(bannerController.bannerList?.isNotEmpty ?? false){
                      ...
                      options: CarouselOptions(autoPlay: true,
                        ...
                        onPageChanged: (index, reason) => bannerController.updateActiveIndex(index),
                      ),
                    ),
                  ),
                  ...
                  Positioned(bottom: 10, left: 0, right: 0,
                    child: Align(
                      ...
                      child:  AnimatedSmoothIndicator(
                        activeIndex: bannerController.getActiveIndex,
                        count: bannerController.bannerList?.length ?? 0,
...
class _BannerShimmerWidget extends StatelessWidget {
  const _BannerShimmerWidget({Key? key}) : super(key: key);
